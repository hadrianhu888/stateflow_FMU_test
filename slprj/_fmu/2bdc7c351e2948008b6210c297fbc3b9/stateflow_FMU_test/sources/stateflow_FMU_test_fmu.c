...
#define FMI2_FUNCTION_PREFIX stateflow_FMU_test_
#endif
/*==========*
 * Includes *
 *==========*/
...
modelData->rOutput[0] = *(modelData->rList[6]);
modelData->rOutput[1] = *(modelData->rList[7]);
modelData->rOutput[2] = *(modelData->rList[8]);
modelData->rOutput[3] = *(modelData->rList[9]);
modelData->rOutput[4] = *(modelData->rList[10]);
modelData->rOutput[5] = *(modelData->rList[11]);
}
void fmu_RestoreOutput() {
    FMUModelData *modelData = (FMUModelData *)_instance;
*(modelData->rList[6]) = modelData->rOutput[0];
*(modelData->rList[7]) = modelData->rOutput[1];
*(modelData->rList[8]) = modelData->rOutput[2];
*(modelData->rList[9]) = modelData->rOutput[3];
*(modelData->rList[10]) = modelData->rOutput[4];
*(modelData->rList[11]) = modelData->rOutput[5];
}
...
fmi2Component fmi2Instantiate(fmi2String instanceName, fmi2Type fmuType, fmi2String fmuGUID,
    ...
    if (!functions->logger) {
        ...
        return NULL;
    } else if (strcmp(fmuGUID, "{bf106fc6-8f71-2bd7-59d2-89e688e6db5b}")) {
        functions->logger(functions->componentEnvironment, instanceName, fmi2Error, "error",
                "fmi2Instantiate: Wrong GUID %s. Expected %s.", fmuGUID, "{bf106fc6-8f71-2bd7-59d2-89e688e6db5b}");
        return NULL;
    }
    ...
    modelData->S = stateflow_FMU_test_M;
    if (modelData->S == NULL) {
        functions->logger(functions->componentEnvironment, instanceName, fmi2Error, "error",
    ...
    modelData->rList[0] = &(stateflow_FMU_test_U.delayCode);
    modelData->rList[1] = &(stateflow_FMU_test_U.offCode);
    modelData->rList[2] = &(stateflow_FMU_test_U.onCode);
    modelData->rList[3] = &(stateflow_FMU_test_U.queueCode);
    modelData->rList[4] = &(stateflow_FMU_test_U.waitCode);
    modelData->rList[5] = &(stateflow_FMU_test_U.startCode);
    modelData->rList[6] = &(stateflow_FMU_test_Y.waitState);
    modelData->rList[7] = &(stateflow_FMU_test_Y.onState);
    modelData->rList[8] = &(stateflow_FMU_test_Y.offState);
    modelData->rList[9] = &(stateflow_FMU_test_Y.delayState);
    modelData->rList[10] = &(stateflow_FMU_test_Y.queueState);
    modelData->rList[11] = &(stateflow_FMU_test_Y.startState);
...
fmi2Status fmi2DoStep(fmi2Component c, fmi2Real currentCommunicationPoint,
    ...
    steps = (int)(communicationStepSize / (double)0.1 + 0.5);    
    modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
            fmi2OK, "info", "CommunicationStepSize=%.16g, LocalSolverStepSize=%.16g", communicationStepSize, (double)0.1);
    if (communicationStepSize < 0 || fabs(communicationStepSize - steps * (double)0.1) > 1.0E-6 * (double)0.1) {
        modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
                    fmi2Error, "error", "Stepsize must be non-negative and divisible by %.16g", (double)0.1);
        returnStatus = fmi2Error;
    } else if (fabs(modelData->time - currentCommunicationPoint) > 1.0E-6 * (double)0.1) {
        modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
